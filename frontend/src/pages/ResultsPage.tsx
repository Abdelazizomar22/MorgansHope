...
export default function ResultsPage({ lang }: ResultsPageProps) {
  ...
  const downloadPDF = () => {
    ...
    const imageURL = result.imagePath
      ? `${import.meta.env.VITE_API_URL?.replace('/api', '') || ''}/uploads/${result.imagePath.split(/[\\/]/).pop()}`
      : '';
...
<div class="results-container">
  <div class="scan-section">
    ...
    <div class="scan-box">
      ${imageURL ? `<img src="${imageURL}" class="scan-img"/>` : '<div style="padding:40px;color:#94a3b8">Scan visual not available</div>'}
