...
export default function ResultsPage({ lang }: ResultsPageProps) {
  ...
  const buildSingleScanHTML = (r: AnalysisResult, base64Image: string) => {
      ...
      <div style="border:1px solid #e2e8f0;border-radius:12px;padding:20px;margin-bottom:20px;page-break-inside:avoid">
        ...
        <div style="display:grid;grid-template-columns:1.2fr 0.8fr;gap:16px">
          <div style="background:#f8fafc;border-radius:10px;padding:12px;text-align:center;border:1px solid #e2e8f0">
            ${base64Image ? `<img src="${base64Image}" style="max-width:100%;max-height:200px;border-radius:7px" />` : '<div style="color:#94a3b8;font-size:12px;padding:30px">Image not available</div>'}
  ...
  const buildReportHTML = (targets: AnalysisResult[], base64Images: string[], form: typeof reportForm, isMulti: boolean) => {
    ...
    return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/>
      ...
      ${targets.map((r, i) => buildSingleScanHTML(r, base64Images[i])).join('')}
  ...
  const printReport = async () => {
    ...
    try {
      ...
      const base64Images = await Promise.all(targets.map(async (r) => {
        const imageURL = r.imagePath ? `${apiBase}/api/uploads/${r.imagePath.split(/[\\/]/).pop()}` : '';
        return imageURL ? await getImageBase64(imageURL) : '';
      }));

      const html = buildReportHTML(targets, base64Images, reportForm, targets.length > 1);
      ...
      alert(t('Failed to generate report.', 'فشل إنشاء التقرير.'));
    } finally {
      setIsPrinting(false);
    }
  };
  ...
  const downloadPDF = async () => {
    ...
    const imageURL = result.imagePath
      ? `${apiBase}/api/uploads/${result.imagePath.split(/[\\/]/).pop()}`
      : '';
...
<div class="results-container">
  <div class="scan-section">
    ...
    <div class="scan-box">
      ${imageBase64 ? `<img src="${imageBase64}" class="scan-img"/>` : '<div style="padding:40px;color:#94a3b8">Scan visual not available</div>'}
            ...
            {!result ? (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 300px', gap: 20 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  <div style={{ background: 'rgba(217,119,6,0.05)', borderRadius: 11, padding: '14px', border: '1px solid rgba(217,119,6,0.3)', display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                    <div style={{ color: '#d97706', flexShrink: 0, marginTop: 1 }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" /><line x1="12" y1="9" x2="12" y2="13" /><line x1="12" y1="17" x2="12.01" y2="17" /></svg></div>
                    <div>
                      <div style={{ fontWeight: 800, color: '#d97706', fontSize: 12, marginBottom: 4 }}>{t('Medical Notice', 'تنبيه طبي')}</div>
                      <div style={{ color: 'var(--text-main)', fontSize: 11.5, lineHeight: 1.6 }}>{t('This AI result is not a final diagnosis. Consult a qualified physician.', 'هذه النتيجة ليست تشخيصاً نهائياً. استشر طبيباً متخصصاً.')}</div>
                    </div>
                  </div>
                </div>
              </div>
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <button
                          ...
                          title={t('Download Report', 'تحميل التقرير')}
                          style={{ padding: '5px 10px', borderRadius: 7, border: '1.5px solid var(--primary)', background: 'transparent', color: 'var(--primary)', display: 'flex', alignItems: 'center', gap: 5, cursor: 'pointer', fontWeight: 700, fontSize: 11, transition: 'all 0.2s' }}
                          onMouseEnter={e => { e.currentTarget.style.background = 'var(--primary)'; e.currentTarget.style.color = 'white'; }}
                          onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--primary)'; }}
                        >
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={() => setShowReportModal(false)}>
            <div style={{ background: 'var(--card-bg)', borderRadius: 18, padding: '32px', width: '100%', maxWidth: 480, boxShadow: '0 24px 60px rgba(0,0,0,0.3)', border: '1px solid var(--card-border)' }} onClick={e => e.stopPropagation()}>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
                ...
                <button onClick={printReport} disabled={isPrinting} style={{ flex: 2, padding: '11px', borderRadius: 9, border: 'none', background: isPrinting ? 'var(--card-border)' : 'var(--primary)', color: isPrinting ? 'var(--text-muted)' : 'white', fontWeight: 700, cursor: isPrinting ? 'default' : 'pointer', fontSize: 13.5, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                  {isPrinting ? t('Processing...', 'جاري التجهيز...') : (
                    <>
